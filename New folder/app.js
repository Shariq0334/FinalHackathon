...
async function store() {
  ...
  let image = await uploadFiless(imageRes.files[0]);
...
function menu() {
  window.location.href = "New folder/menu.html";
}

firebase.auth().onAuthStateChanged((user) => {
        // var uid = user.uid;
        firebase.database().ref(`users`).once('child_added', (data) => {
            let restaurant = document.getElementById('container')
            restaurant.innerHTML += `
            <div class="card" style="width: 18rem; margin-bottom:10px">
            <img width="250px" class="card-img-top" src='${data.val().pic}' alt="Card image cap">
            <div class="card-body">
              <h1 class="card-title">${data.val().userName}</h1>
              <h5>CHECK OUT OUR DELICIOUS DISHES ORDER NOW</h5>
              <h3 class="card-text">${data.val().country}</h3>
             <h3> <a  href="New folder/menu.html?restaurant=${data.key}" class="btn btn-primary">See our Menu</a></h3>
            </div>
          </div>
            `   
          })
